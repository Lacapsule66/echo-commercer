...
const PreviewSliderModal = () => {
  ...
  return (
    ...
    >
      ...
      <div>
        <button
          className="rotate-180 absolute right-[80%] p-5 cursor-pointer z-10 "
          onClick={handlePrev}
        >
          <svg
            width="36"
        ...
        <button
          className="absolute left-[80%] p-5 cursor-pointer z-10"
          onClick={handleNext}
        >
          <svg
            width="36"
      ...
      <Swiper ref={sliderRef} slidesPerView={1} spaceBetween={20}>
        {data.previewImages?.map((img: any, key:number) => (
          <SwiperSlide key={key}>
            <div className="flex justify-center items-center">
              <Image
                src={img.image ? imageBuilder(img?.image).url()! : "/"}
                alt={data.name}
                width={450}
                height={450}
              />
